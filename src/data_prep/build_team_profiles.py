...
def load_data():
    ...
    logger.info(f"Loaded: {len(matches)} matches, {len(lineups)} lineups, "
...
def get_player_prior_stats(player_stats: pd.DataFrame, player_id: int,
                           wc_year: int, lookback_years: int = 3) -> pd.Series:
    """
    ...
    rate_cols = ['rating', 'passes_accuracy', 'goals_per_90', 'assists_per_90',
                 'goal_contributions_per_90', 'shot_accuracy', 'duel_win_pct',
                 'tackles_per_90', 'dribble_success_pct']
    result = {}
...
def build_team_profile(lineups: pd.DataFrame, player_stats: pd.DataFrame,
                       fixture_id: int, team_id: int, wc_year: int) -> dict:
    """
    ...
    stat_keys = {
        'goals': 'sum', 'assists': 'sum', 'appearances': 'sum', 'minutes': 'sum',
        'rating': 'mean', 'passes_accuracy': 'mean',
        'goals_per_90': 'mean', 'assists_per_90': 'mean',
        'goal_contributions_per_90': 'mean',
        'tackles_per_90': 'mean', 'duel_win_pct': 'mean',
        'shots_total': 'sum', 'shots_on_target': 'sum',
        'cards_yellow': 'sum', 'cards_red': 'sum',
    ...
    # Position breakdown
    positions = profiles_df['position'].value_counts().to_dict() if 'position' in profiles_df.columns else {}
    ...
    for _, p in profiles_df.iterrows():
        ...
        for col in ['goals', 'assists', 'appearances', 'rating', 'minutes',
                     'goals_per_90', 'passes_accuracy']:
...
def get_team_prior_wc_stats(team_stats: pd.DataFrame, team_name: str,
                            wc_year: int) -> list:
    """
...
def get_h2h_record(matches: pd.DataFrame, team1: str, team2: str,
                   before_year: int) -> dict:
...
def build_match_context(match_row: pd.Series, lineups: pd.DataFrame,
                        player_stats: pd.DataFrame, team_stats: pd.DataFrame,
                        events: pd.DataFrame, matches: pd.DataFrame) -> dict:
    """
