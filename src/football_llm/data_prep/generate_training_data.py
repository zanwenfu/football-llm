...
from football_llm.paths import PROCESSED_DIR, TRAINING_DIR
...
def format_team_compact(profile: dict, team_label: str) -> str:
    ...
    pos_str = ", ".join(
        ...
        for k, count in sorted(
            pos.items(),
            key=lambda x: (
                ["Goalkeeper", "Defender", "Midfielder", "Attacker", "Forward"].index(x[0])
                if x[0] in ["Goalkeeper", "Defender", "Midfielder", "Attacker", "Forward"]
                else 99
            ),
        )
    )
...
def build_compact_reasoning(context: dict, anonymize: bool = False) -> str:
    ...
    if h_goals > 0 and a_goals > 0:
        if h_goals > a_goals * 1.15:
            parts.append(
                f"{home}'s squad has higher goal output ({int(h_goals)} vs {int(a_goals)})"
            )
        elif a_goals > h_goals * 1.15:
            parts.append(
                f"{away}'s squad has higher goal output ({int(a_goals)} vs {int(h_goals)})"
            )

    if h_rating > 0 and a_rating > 0 and abs(h_rating - a_rating) > 0.15:
        better = home if h_rating > a_rating else away
        parts.append(
            f"{better} has a higher average rating ({max(h_rating, a_rating):.1f} vs {min(h_rating, a_rating):.1f})"
        )

    if h_top > 0 and a_top > 0 and abs(h_top - a_top) > 5:
        better = home if h_top > a_top else away
        parts.append(
            f"{better}'s top scorer is more prolific ({int(max(h_top, a_top))} vs {int(min(h_top, a_top))} goals)"
        )

    if h_pass > 0 and a_pass > 0 and abs(h_pass - a_pass) > 3:
        better = home if h_pass > a_pass else away
        parts.append(
            f"{better} has better passing ({max(h_pass, a_pass):.0f}% vs {min(h_pass, a_pass):.0f}%)"
        )
    ...
    if not parts:
        if result == "draw":
            parts.append(
                "Both teams have closely matched statistical profiles suggesting an even contest"
            )
        else:
