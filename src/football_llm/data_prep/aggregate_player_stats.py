...
def load_statistics_clean() -> pd.DataFrame:
    ...
    path = os.path.join(
        RAW_DIR, "player_stats", "statistics_clean", "all_player_statistics_clean.csv"
    )
    df = pd.read_csv(path)
...
def merge_player_stats(career: pd.DataFrame, stats_clean: pd.DataFrame) -> pd.DataFrame:
    ...
    )  # stats_clean < wc_career alphabetically...
    ...
    combined = combined.drop_duplicates(
        subset=["player_id", "season", "team_id", "league_id"], keep="first"
    )
...
def aggregate_player_seasons(df: pd.DataFrame) -> pd.DataFrame:
    ...
    meta = (
        df.sort_values("season").groupby("player_id")[meta_available].first().reset_index(drop=True)
    )
...
def compute_derived_stats(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if "shots_on_target" in df.columns and "shots_total" in df.columns:
        df["shot_accuracy"] = (
            df["shots_on_target"] / df["shots_total"].replace(0, np.nan) * 100
        ).round(1)
    ...
    if "dribbles_success" in df.columns and "dribbles_attempts" in df.columns:
        df["dribble_success_pct"] = (
            df["dribbles_success"] / df["dribbles_attempts"].replace(0, np.nan) * 100
        ).round(1)
