...
class ScriptArguments:
    ...
    eval_dataset_path: str | None = field(
        default=None,
        metadata={
            "help": "Path to evaluation JSONL/JSON file. If not provided, no eval is run during training."
        },
    )
    ...
    spectrum_config_path: str | None = field(
        default=None,
...
def setup_model_for_spectrum(model, spectrum_config_path):
    ...
    """
    unfrozen_parameters = []
    with open(spectrum_config_path) as fin:
...
def train_function(model_args: ModelConfig, script_args: ScriptArguments, training_args: SFTConfig):
    ...
    model_kwargs = dict(
        ...
        low_cpu_mem_usage=(
            True if not strtobool(os.environ.get("ACCELERATE_USE_DEEPSPEED", "false")) else None
        ),
    )
    ...
    if trainer.accelerator.is_main_process:
        trainer.create_model_card(
            {"tags": ["sft", "football-llm", "world-cup-prediction", "qlora"]}
        )
